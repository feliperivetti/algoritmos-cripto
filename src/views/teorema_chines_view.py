    ...
    def render(self, result: AlgorithmResult) -> None:
        ...
        if terms:
            ...
            for term in terms:
                ...
                st.markdown(
                    f"**Termo {term['i']}** "
                    f"(para $a_{term['i']}={term['a_i']}, m_{term['i']}={term['m_i']}$)"
                )
                st.write(f"- $M_{term['i']} = M / m_{term['i']} = {term['M_i']}$")
                st.write(
                    rf"- $y_{term['i']} = (M_{term['i']})^{{-1}} "
                    rf"\pmod{{m_{term['i']}}} = {term['y_i']}$"
                )
                st.write(
                    rf"- Parcela: ${term['a_i']} \cdot {term['M_i']} "
                    rf"\cdot {term['y_i']} = {term['term']}$"
                )
